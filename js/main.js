function myFunction(x) {
  ...
  {
    const color=['#00431a','#631c2f','#aa3f58','#8f2907'];
    if(i==4)i=0;

    document.querySelector(".svgClass").getSVGDocument().getElementById('internalID').setAttribute("fill",color[i]);
    document.getElementById('nav-head').style.backgroundColor=color[i];
    document.getElementById('org').style.backgroundColor=color[i];
     document.getElementById('home').style.backgroundColor=color[i++];
    
    setTimeout(changeBgColor,3000,i);
  }